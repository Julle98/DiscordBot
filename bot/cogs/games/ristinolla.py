...
class TicTacToeButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if winner:
            ...
            games_utils.add_xp(winner.id, 10)
        ...
        if all(cell != "" for row in view.board for cell in row):
            games_utils.add_xp(view.player_x.id, 2)
            games_utils.add_xp(view.player_o.id, 2)
            for b in view.children:
                b.disabled = True
            await interaction.response.edit_message(view=view)
...
class Ristinolla(commands.Cog):
    ...
    async def peli_ristinolla(self, interaction: discord.Interaction, opponent: discord.Member):
        ...
        asyncio.create_task(kirjaa_komento_lokiin(self.bot, interaction, "/peli_ristinolla"))
        asyncio.create_task(kirjaa_ga_event(self.bot, interaction.user.id, "peli_ristinolla_komento"))
