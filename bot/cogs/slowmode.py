...
class SlowmodeTracker(commands.Cog):
    ...
    async def initialize_slowmode_state(self):
        ...
        if current_delay == self.high_slowmode:
            ...
            await self.send_embed(console_channel or channel, self.low_slowmode, raised=False, count=0, old_delay=current_delay)
        ...
        if desired_state != self.last_slowmode:
            if self.pending_state == desired_state:
                ...
                if self.pending_count >= 2:
                    old_delay = self.last_slowmode
                    await channel.edit(slowmode_delay=desired_state)
                    await self.send_embed(channel, desired_state, raised=(desired_state == self.high_slowmode), count=count, old_delay=old_delay)
                    if console_channel:
                        await self.send_embed(console_channel, desired_state, raised=(desired_state == self.high_slowmode), count=count, old_delay=old_delay)
    ...
    @app_commands.command(name="setslowmode", description="Muuta #Yleinen kanavan etanatilaa manuaalisesti")
    ...
    async def set_slowmode(self, interaction: discord.Interaction, seconds: int):
        ...
        if not channel:
            await interaction.response.send_message("❌ Etanatila-kanavaa ei löytynyt.", ephemeral=True)
            return

        old_delay = channel.slowmode_delay
        await channel.edit(slowmode_delay=seconds)

        raised = seconds > old_delay
        self.last_slowmode = seconds

        await self.send_embed(channel, seconds, raised, count=0, old_delay=old_delay)
        if console_channel:
            await self.send_embed(console_channel, seconds, raised, count=0, old_delay=old_delay)

        await interaction.response.send_message(f"✅ Etanatila asetettu {seconds} sekuntiin (vanha {old_delay}s).", ephemeral=True)

    async def send_embed(self, channel, delay, raised, count, old_delay=None):
        now_ts = time.time()
        ...
        old_info = f"(vanha: {old_delay}s)" if old_delay is not None else ""

        description = (
            f"**{direction}** – uusi viestirajoitus: **{delay} sekuntia** {old_info}\n"
